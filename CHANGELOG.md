...
- 自动识别页面类型与轮次（基于页面自带的 `teachingClassType` 与批次名称），切换页面时自动跟随。
- 轮次差异化：仅第一轮通识选修为志愿摇号制（显示志愿选、固定 1 并发）；方案内课程与第二轮正选为即选即得，按配置并发。
- README 使用指南与 FAQ。
- 状态机纯逻辑抽取（释放认领、剩余跳过、志愿选择、已选校验）及单元测试（共 28 个）。
- 面板视觉焕新：玻璃质感卡片、渐变按钮、状态胶囊徽章、区分类型的彩色标签。

### Changed

- 面板文案全面以 Celeritas 品牌呈现。
- GitHub Actions 升级至最新主版本（checkout v7 / setup-node v7 / action-setup v6）并覆盖所有推送分支。

### Fixed

- 搜索按钮白底白字与内容被挤压导致的不可见。
