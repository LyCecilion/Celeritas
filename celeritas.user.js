...
`;
    ...
    // ===================== 状态 =====================
    let courses = []; // { code, name, note, jxbid?, secretVal?, type? }  jxbid为空=任意班; type: XGKC=通识选修 / TYKC=体育俱乐部
    let interval = 800;
    let concurNum = 2; // 并发数
    let courseType = "XGKC"; // 当前面板课程类型: XGKC=通识选修, TYKC=体育俱乐部, TJKC=推荐班级课程
    let volunteer = 1; // 预选批次志愿（1~5，摇号制）
    let running = false;
    let stopped = false;
    let grabbed = []; // [code, ...] 真正抢到的
    let skipped = []; // [code, ...] 被跳过的
    let claimed = []; // [code, ...] 已被 worker 认领的
    let creditsFull = false; // 学分/门数超限标志
    let workerTargets = {}; // { workerId: { code, idx } }  当前各 worker 处理的课程
    ...
    function loadConfig() {
        try {
            ...
            // 首次加载：迁入旧版 ccb_courses_* 存储的课程，迁移后删除旧 key
            if (!raw) {
            ...
            if (raw) {
                ...
                courses = (data.courses || []).map(function (c) {
                    ...
                    // 兼容旧数据：无 type/sportName/volunteer 字段的课程默认通识选修/第一志愿
                    c.type = c.type || "XGKC";
    ...
    function log(msg, type) {
        const el = document.getElementById("clrt-log");
        if (!el) return;
        const now = new Date();
        const ts =
    ...
    function renderCourseList() {
        const el = document.getElementById("clrt-course-list");
        if (!el) return;
    ...
    function updateProgress() {
        const el = document.getElementById("clrt-progress");
        const remainEl = document.getElementById("clrt-remaining");
    ...
    function updateBtnState() {
        const startBtn = document.getElementById("clrt-btn-start");
        const stopBtn = document.getElementById("clrt-btn-stop");
        const skipBtn = document.getElementById("clrt-btn-skip");
    ...
    function updateTypeHint() {
        const input = document.getElementById("clrt-input-keyword");
        const hint = document.getElementById("clrt-type-hint");
        if (input) {
    ...
    // 从 API 响应中提取 rows（兼容 data 为数组或 {rows} 对象两种格式）
    ...
    // 当前是否为预选批次（第一批：志愿摇号制，提交带 chooseVolunteer）
    ...
    // 将 API 返回的行展开为教学班行：TYKC/TJKC 结构为课程对象 + tcList 教学班子数组
    ...
    async function addCourse(courseObj, type, fallbackSecret, volunteerGrade) {
        ...
        try {
            if (clazzType === "TYKC" || clazzType === "TJKC") {
                // 方案内课程（体育俱乐部/推荐班级课程）：与页面"选课"按钮一致，使用表单格式提交
    ...
    // 验证方案内课程（体育俱乐部/推荐班级课程）是否真正选上（查真实已选列表）
    async function verifyBySelect(code, jxbid, sportName, maxRetries) {
        ...
        for (let i = 0; i < maxRetries; i++) {
            ...
            const found = rows.find(function (r) {
                ...
                if (jxbid && jx) return jx === jxbid; // 指定教学班：精确匹配
                if (sportName && sn) return sn === sportName; // 指定俱乐部：按名称匹配
    ...
    // 单课程抢课（worker 内调用，返回 true=成功或被跳过）
            ...
            try {
                ...
                if (sections.length > 0) {
                    ...
                    for (let si = 0; si < candidates.length && !stopped; si++) {
                        ...
                        // 预选批次（第一批通识，志愿摇号制）：先查可选志愿，提交带 chooseVolunteer
...
<div class="clrt-body" id="clrt-body">
    ...
    function makeDraggable() {
        ...
        const header = document.getElementById("clrt-header");
    ...
    // ===================== 事件绑定 =====================
    function bindEvents() {
        const keywordEl = document.getElementById("clrt-input-keyword");
        const searchBtn = document.getElementById("clrt-btn-search");
        const resultsEl = document.getElementById("clrt-search-results");
        ...
        document.getElementById("clrt-btn-start").addEventListener("click", startGrab);
        document.getElementById("clrt-btn-stop").addEventListener("click", stopGrab);
        document.getElementById("clrt-btn-skip").addEventListener("click", skipCourse);

        document.getElementById("clrt-interval").addEventListener("change", function () {
            interval = parseInt(this.value);
            saveConfig();
        });

        document.getElementById("clrt-concur").addEventListener("change", function () {
            concurNum = parseInt(this.value);
            saveConfig();
        });

        document.getElementById("clrt-type").addEventListener("change", function () {
        ...
        document.getElementById("clrt-volunteer").addEventListener("change", function () {
            volunteer = parseInt(this.value);
            saveConfig();
        });

        // 最小化 / 关闭
        document.getElementById("clrt-btn-min").addEventListener("click", function () {
            const body = document.getElementById("clrt-body");
            const btn = document.getElementById("clrt-btn-min");
        ...
        document.getElementById("clrt-btn-close").addEventListener("click", function () {
            if (running) {
    ...
    function showReopenBtn() {
        if (document.getElementById("clrt-reopen")) return;
    ...
    function init() {
        ...
        // 恢复下拉默认值
        const sel = document.getElementById("clrt-interval");
        if (sel) sel.value = String(interval);
        const concurSel = document.getElementById("clrt-concur");
        if (concurSel) concurSel.value = String(concurNum);
        const typeSel = document.getElementById("clrt-type");
        if (typeSel) typeSel.value = courseType;
        const volSel = document.getElementById("clrt-volunteer");
